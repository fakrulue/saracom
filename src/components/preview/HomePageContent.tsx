...
export function HomePageContent({ config }: HomePageContentProps) {
  ...
  const saleHandle = sections?.sale?.collectionHandle;
  const bestHandle = sections?.bestSellers?.collectionHandle;
  const newArrHandle = sections?.newArrivals?.collectionHandle;

  useEffect(() => { setMounted(true); }, []);

  useEffect(() => {
    const handles = [saleHandle, bestHandle, newArrHandle].filter(Boolean) as string[];
...
}, [saleHandle, bestHandle, newArrHandle]);
  ...
  return (
    <div onClick={() => onSelectSection("")}>
      {sections.topBanner?.enabled && (
        <ClickableSection id="topBanner">
          <div className="bg-gradient-to-r from-pink-500 to-rose-500 py-2 px-4 text-center">
            <a href={sections.topBanner?.link} className="text-xs font-bold text-white hover:opacity-80">
              {sections.topBanner?.text}
            </a>
          </div>
        </ClickableSection>
      )}
          ...
          <section className="relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${p}15, ${a}15)` }}>
            <div className="mx-auto grid max-w-7xl items-center gap-6 px-4 py-8 sm:py-12 lg:grid-cols-2 lg:gap-8 lg:py-20">
              <div className="space-y-4 sm:space-y-5">
                ...
                <h1 className="font-extrabold leading-[1.05] sm:text-5xl md:text-6xl" style={{ fontFamily: hf, fontSize: "clamp(2rem, 8vw, 60px)", color: t }}>
                  {sections.hero?.title}
                </h1>
      ...
      {sections.trustStrip?.enabled && (
        <ClickableSection id="trustStrip">
      ...
      {sections.categories?.enabled && (
        <ClickableSection id="categories">
      ...
      {sections.bestSellers?.enabled && (
        <ClickableSection id="bestSellers">
          <section className="mx-auto max-w-7xl px-4 py-10 sm:py-16">
            <div className="mb-6 flex flex-col gap-1 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
              <div>
      ...
      {sections.newArrivals?.enabled && (
        <ClickableSection id="newArrivals">
      ...
      {sections.videoReviews?.enabled && (
        <ClickableSection id="videoReviews">
          <VideoReviewsSection />
        </ClickableSection>
      )}

      {sections.newsletter?.enabled && (
        <ClickableSection id="newsletter">
