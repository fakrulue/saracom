      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-background shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <Link href="/collections/all" onClick={close} className="mt-4 inline-block rounded-full bg-brand-pink px-5 py-2 text-sm font-bold text-white">
                Continue shopping
              </Link>
            </div>
          ) : (
        ...
        {items.length > 0 && (
          <footer className="border-t p-5 space-y-3">
            ...
            <Link href="/cart" onClick={close} className="block text-center text-xs underline text-muted-foreground">
              View full cart
            </Link>
          </footer>
        )}
