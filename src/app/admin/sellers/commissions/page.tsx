...
export default function CommissionsPage() {
  ...
  const avgRate = commissions.length > 0 
    ? (commissions.reduce((sum, c) => sum + (c.rateType === "percentage" ? c.rate : 0), 0) / commissions.filter(c => c.rateType === "percentage").length || 0).toFixed(1)
    : "0";
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard title="Avg. Commission Rate" value={loading ? "—" : `${avgRate}%`} icon={Percent} color="text-emerald-600 bg-emerald-50" />
          <StatCard title="Revenue Share (MTD)" value="—" icon={TrendingUp} color="text-blue-600 bg-blue-50" />
          <StatCard title="Active Rules" value={loading ? "—" : String(activeCount)} icon={ShieldCheck} color="text-violet-600 bg-violet-50" />
        </div>
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-500">No commission rules found</p>
            </div>
          ) : (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{rule.category || "All Categories"}</p>
                        </div>
                      </div>
                      <Badge className={cn(
                    ...
                    <div className="flex items-center justify-between pt-4 border-t">
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900">{rule.rateType === "percentage" ? "Percentage" : "Fixed"}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Commission</p>
                        <p className="text-2xl font-black text-slate-900">
                          {rule.rateType === "percentage" ? `${rule.rate}%` : `$${rule.rate.toFixed(2)}`}
                        </p>
                      </div>
                    </div>
