      ...
      {items.length === 0 ? (
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
            ...
            {items.map((item) => (
              <div key={item.productId + (item.size ?? "")} className="flex gap-4 rounded-2xl border p-4">
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/products/${item.handle}`} className="font-semibold hover:underline">{item.title}</Link>
          ...
          <aside className="h-fit rounded-2xl border p-6">
            ...
            <Link href="/checkout" className="mt-6 block rounded-full bg-brand-pink py-3 text-center text-sm font-bold text-white">Check out</Link>
          </aside>
        </div>
      )}
    </div>
